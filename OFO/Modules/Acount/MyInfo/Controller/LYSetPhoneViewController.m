...
@interface LYSetPhoneViewController ()
...
@implementation LYSetPhoneViewController
...
- (IBAction)PhoneTextfieldDidEditing:(UITextField *)sender {
    
    if ([sender.text isPhone]) {
        self.codeBtn.enabled=YES;
        self.codeBtn.backgroundColor=mainColor;
    }else{
        self.codeBtn.enabled=NO;
        self.codeBtn.backgroundColor=ColorWithRGB(219, 219, 219);
    }
...
- (IBAction)confirm:(UIButton *)sender {
    ...
    if (![[LYDataManager shareManager]isExistWithPhone:self.phoneTF.text]) {
        [SMSSDK commitVerificationCode:self.codeTF.text phoneNumber:self.phoneTF.text zone:@"86" result:^(NSError *error) {
            if (!error) {
                ...
                [NSTimer scheduledTimerWithTimeInterval:3.0 repeats:NO block:^(NSTimer * _Nonnull timer) {
                    ...
                    [[LYDataManager shareManager]updateUserInfoWithKey:@"userPhone" AndValue:self.phoneTF.text];
                    [[NSUserDefaults standardUserDefaults]setObject:self.phoneTF.text forKey:@"userPhone"];
                    [[NSUserDefaults standardUserDefaults]synchronize];
                    [[NSNotificationCenter defaultCenter]postNotificationName:ResetPhoneSuccessNotification object:nil];
                    for (UIViewController *controller in self.navigationController.viewControllers) {
                        if ([controller isKindOfClass:[LYMyInfoViewController class]]) {
                            LYMyInfoViewController *A =(LYMyInfoViewController *)controller;
